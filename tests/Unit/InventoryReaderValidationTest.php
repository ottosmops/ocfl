...
test('rejects missing id with E036', function (): void {
    expectOcflErrorCode(
        fn () => InventoryReader::fromFile(badFixture('E036_no_id')),
        ErrorCode::E036,
    );
});
...
test('rejects missing head with E036', function (): void {
    expectOcflErrorCode(
        fn () => InventoryReader::fromFile(badFixture('E036_no_head')),
        ErrorCode::E036,
    );
});
...
test('rejects missing manifest with E041', function (): void {
    expectOcflErrorCode(
        fn () => InventoryReader::fromFile(badFixture('E041_no_manifest')),
        ErrorCode::E041,
    );
});
...
test('rejects wrong digest algorithm with E025', function (): void {
    expectOcflErrorCode(
        fn () => InventoryReader::fromFile(badFixture('E025_wrong_digest_algorithm')),
        ErrorCode::E025,
    );
});
...
test('rejects empty versions object with E036', function (): void {
    expectOcflErrorCode(
        fn () => InventoryReader::fromFile(badFixture('E008_E036_no_versions_no_head')),
        ErrorCode::E036,
    );
});
...
test('rejects non-readable file with E033', function (): void {
    expectOcflErrorCode(
        fn () => InventoryReader::fromFile('/tmp/no-such-inventory-' . uniqid() . '.json'),
        ErrorCode::E033,
    );
});
